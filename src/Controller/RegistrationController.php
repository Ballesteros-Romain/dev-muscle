...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, UserAuthenticator $authenticator, EntityManagerInterface $entityManager, SendMailService $mail, JWTService $jwt): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            //on génère la payload avec les informations du user
            $payload = [
                'user_id' => $user->getId()
            ];
            // on genere le token
    ...
    public function verifyUser($token, JWTService $jwt, UserRepository $userRepository, EntityManager $em): Response
    {
        // on verifie si le token est valide, n'a pas expire et n'a pas ete modiife
        if (!$jwt->isValid($token) && !$jwt->isExpired($token) && $jwt->check($token, $this->getParameter('app.jwtsecret'))) {
            ...
            if($user && !$user->getIsVerified()){
                ...
                $em->flush($user);
                $this->addFlash("success", "Votre compte a bien été activer");
                return $this->redirectToRoute('profile_index');
            }
    }
        // ici un probleme ce pose dans le token 
        $this->addFlash('danger', 'Letoken est invalide ou à expiré');
